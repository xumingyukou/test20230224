...
func run(t *testing.T, lim *Limiter, allows []allow) {
	...
	for i, allow := range allows {
		sawDivergence := false
		for j := 0; j < allow.n; j++ {
			r := lim.reserve(allow.t, 1)
			t.Logf("step %d: lim.AllowN(%v, %v) = %v want %v",
				i, allow.t, allow.n, r, allow.ok)
...
func runN(t *testing.T, lim *Limiter, allows []allow) {
	t.Helper()
	for i, allow := range allows {
		r := lim.reserve(allow.t, int32(allow.n))
...
func TestLimiterBurstN1(t *testing.T) {
	runN(t, NewLimiter(20, 3), []allow{
		{t0, 3, true},
		{t0, 1, false},
		{t0, 2, false},
		{t1, 2, true},
...
func TestLimiter_noTruncationErrors(t *testing.T) {
	if NewLimiter(0.7692307692307693, 1).Allow() < 0 {
