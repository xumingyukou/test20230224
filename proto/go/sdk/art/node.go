...
func (an *artNode) findChildRead(c byte, valid bool) (*artNode, *artNode) {
	node := an.node()

	if !valid {
		return node.zeroChild, node.zeroChild
	}
	...
	if idx != -1 {
		switch an.kind {
		case Node4:
			return node.zeroChild, an.node4().children[idx]

		case Node16:
			return node.zeroChild, an.node16().children[idx]

		case Node48:
			return node.zeroChild, an.node48().children[idx]

		case Node256:
			return node.zeroChild, an.node256().children[idx]
		}
	}

	return node.zeroChild, nodeNotFound
}
