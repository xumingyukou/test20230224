...
func (t *tree) Search(key Key) (Value, bool) {
	...
	for current != nil {
		...
		_, next := current.findChildRead(key.charAt(int(depth)), key.valid(int(depth)))
		if next != nil {
			current = next
		} else {
			current = nil
...
func (t *tree) LongestSearch(key Key) (Key, Value, bool) {
	var last *leaf
	current := t.root
	depth := uint32(0)
	for current != nil {
		...
		zero, next := current.findChildRead(key.charAt(int(depth)), key.valid(int(depth)))
...
func (t *tree) SearchPrefixCb(key Key, cb func(k Key, v Value) bool) (done bool) {
	current := t.root
	depth := uint32(0)
	done = true
	for current != nil {
		if current.isLeaf() {
			leaf := current.leaf()
			if bytes.HasPrefix(key, leaf.key) {
				done = cb(leaf.key, leaf.value)
				if !done || len(key) == len(leaf.key) {
					break
				}
			}
		}
