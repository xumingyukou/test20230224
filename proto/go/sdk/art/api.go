...
type Tree interface {
	...
	// Search returns the value of the longest prefix match with key.
	// If the prefix match then return key, value, true and nil, nil, false otherwise.
	LongestSearch(key Key) (longestKey Key, value Value, found bool)

	// Search all keys which is prefix of input key, and call the cb
	SearchPrefixCb(key Key, cb func(k Key, v Value) bool) (done bool)
