...
// Return reamining quota for next second
func (lim *Limiter) Allow() (remain int32) {
	return lim.reserve(time.Now(), 1)
}

// Return reamining quota for next second
func (lim *Limiter) AllowN(n int32) (remain int32) {
	return lim.reserve(time.Now(), n)
}

// reserve is a helper method for Allow -- in particular it is used in testing where
// time is mocked.
func (lim *Limiter) reserve(now time.Time, n int32) (remain int32) {
	if lim.limit == Inf {
		return math.MaxInt32
	} else if lim.limit == 0 {
		if lim.burst >= int64(n) {
			newBurst := atomic.AddInt64(&lim.burst, -int64(n))
			return int32(newBurst)
		} else {
			return -1
		}
	}
	...
	for i := 0; i < maxTries; i++ {
		...
		if nowMicros == lastUpdateMicros {
			if tokens < n {
				...
				// to the le ft of token count, if we drop below 0 tokens here when racing
				// it is fine.
				//
				// This branch is mostly hit if we have burst capacity and a bunch of
				// concurrent requests coming in at once.
				newPackedState := packedState(atomic.AddUint64(&lim.state, (^uint64(0))-uint64(n-1)))
