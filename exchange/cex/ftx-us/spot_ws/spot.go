...
LOOP:
	for {
		select {
		case <-ctx.Done():
			if err := b.Unsubscribe(wsClient, request, symbol.Symbol); err != nil {
		...
		case status, ok := <-ch:
			...
			logger.Logger.Error("reconnecting", symbol.Symbol)
			if err := b.Unsubscribe(wsClient, request, symbol.Symbol); err != nil {
				logger.Logger.Error("unsubscribe err:", symbol.Symbol, err)
			}
			if err := b.Subscribe(wsClient, request, symbol.Symbol); err != nil {
...
func (b *FTXSpotWebsocket) FundingRateGroup(context.Context, map[*client.SymbolInfo]chan *client.WsFundingRateRsp) error {
