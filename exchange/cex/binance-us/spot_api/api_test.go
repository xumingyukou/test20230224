...
func TestGetSymbols(t *testing.T) {
	if res, err := apiClient.ExchangeInfo(); err != nil {
		t.Log(err)
	} else {
		fmt.Println(len(res.Symbols))
		for _, symbol := range res.Symbols {
			if symbol.IsSpotTradingAllowed {
				fmt.Printf("%s/%s, ", symbol.BaseAsset, symbol.QuoteAsset)
			} else {
				fmt.Printf("!%s/%s, ", symbol.BaseAsset, symbol.QuoteAsset)
			}
		}
...
func TestTrade(t *testing.T) {
	...
	res, err := apiClient.Order("BUSDUSDT", spot_api.SIDE_TYPE_SELL, spot_api.ORDER_TYPE_MARKET, &params)
	if err != nil {
		t.Fatal(err)
	}
	resp, ok := res.(*spot_api.RespMarginOrderFull)
	if !ok {
...
func TestLimitTrade(t *testing.T) {
	...
	res, err := apiClient.Order("BUSDUSDT", spot_api.SIDE_TYPE_BUY, spot_api.ORDER_TYPE_LIMIT, &params)
	if err != nil {
		t.Fatal(err)
	}
	resp, ok := res.(*spot_api.RespMarginOrderFull)
...
func TestAssetTransfer(t *testing.T) {
	...
	res, err := apiClient.AssetTransfer(spot_api.MOVE_TYPE_MARGIN_FUNDING, asset, amount, &params)
	if err != nil {
		t.Fatal(err)
	}
	fmt.Println("res:", res)
