...
func TestCheckFullDepth(t *testing.T) {
	...
	h.SetDepthIncrementSnapShotConf([]*client.SymbolInfo{symbolInfo}, conf)
	go h.checkFullDepth(symbolInfo)
	var ch chan *base.OrderBook
	sym := SymbolKeyGen(symbolInfo.Symbol, symbolInfo.Market, symbolInfo.Type)
	if chI, ok := h.CheckDepthCacheChanMap.Get(sym); ok {
		ch, ok = chI.(chan *base.OrderBook)
		if !ok {
			fmt.Println("get CheckDepthCacheChanMap err", sym)
			return
		}
	} else {
		fmt.Println("get CheckDepthCacheChanMap err", sym)
